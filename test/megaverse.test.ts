...
import type { AstralLiteral } from '../src/models/api/astral.js';
import type { LiteralMap } from '../src/models/api/map.js';
...
describe('Megaverse', () => {
    ...
    const megaverse = new Megaverse(
        'test-candidate',
        mockPolyanetService as any,
        mockSoloonService as any,
        mockComethService as any
    );
    ...
    test('should add new objects to empty map', async () => {
        ...
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 0, 0);
        expect(mockSoloonService.add).toHaveBeenCalledWith('test-candidate', 0, 1, 'red');
        expect(mockComethService.add).toHaveBeenCalledWith('test-candidate', 1, 0, 'up');
    });
    ...
    test('should replace existing objects with new ones', async () => {
        ...
        const candidateMap = [
            [new Soloon('red'), new Polyanet()],
            [new Cometh('up'), SpaceObject]
        ];
        ...
        // Verify deletions
        expect(mockSoloonService.delete).toHaveBeenCalledWith('test-candidate', 0, 0);
        expect(mockPolyanetService.delete).toHaveBeenCalledWith('test-candidate', 0, 1);
        expect(mockComethService.delete).toHaveBeenCalledWith('test-candidate', 1, 0);

        // Verify additions
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 0, 0);
        expect(mockSoloonService.add).toHaveBeenCalledWith('test-candidate', 0, 1, 'blue');
        expect(mockComethService.add).toHaveBeenCalledWith('test-candidate', 1, 0, 'down');
    });
    ...
    test('should handle cross correctly', async () => {
        ...
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 2, 2);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 3, 3);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 4, 4);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 5, 5);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 6, 6);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 7, 7);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 8, 8);

        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 2, 8);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 3, 7);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 4, 6);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 6, 4);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 7, 3);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 8, 2);
    });
    ...
    test('should handle crossmint logo correctly', async () => {
        ...
        // Verify service calls for some key positions
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 2, 2);
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 2, 3);
        expect(mockComethService.add).toHaveBeenCalledWith('test-candidate', 1, 7, 'right');
        expect(mockSoloonService.add).toHaveBeenCalledWith('test-candidate', 4, 5, 'blue');
        expect(mockSoloonService.add).toHaveBeenCalledWith('test-candidate', 4, 8, 'purple');
        expect(mockPolyanetService.add).toHaveBeenCalledWith('test-candidate', 3, 21);
    });
}); 